...
import LoginGuest from "../lib/mutation/Guest/Login";
import LoginUser from "../lib/mutation/User/Login";
...
function Login() {
  ...
  const {mutate:Guest}=LoginGuest();
  const {mutate:User}=LoginUser();
  const onSubmit = (data) => {
    const payload={
      email:data?.email,
  ...
  return (
    <>
      <div className="w-[1200px] max-w-full h-[calc(100vh-36px)] mx-auto bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 p-4 md:p-6 rounded-xl">
          ...
          <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="flex items-center">
              <input
                ...
                {...register("remember")} // Add register for remember me
              />
            ...
            >
              <span className="material-symbols-outlined mr-2">Login as Guest</span> {/* Removed guest login button */}
            </button>
            <button
              type="submit"
              className="w-full px-4 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg hover:from-indigo-700 hover:to-purple-700 transition-all duration-300 transform hover:-translate-y-0.5 shadow-lg hover:shadow-xl flex items-center justify-center"
            >
              <span className="material-symbols-outlined mr-2">Login</span> {/* Removed guest login button */}
            </button>
